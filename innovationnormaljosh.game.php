...
class InnovationNormaljosh extends Table
{
...
<<<<<<< HEAD
    ...
    //     }
    //     else if ($card['location'] == 'achievements') {
    //         throw new BgaUserException(self::_("This card is already used as an achievement"));
    //     }
    //     else if ($card['location'] == 'removed') {
    //         throw new BgaUserException(self::_("This card is removed from the game"));
    //     }
    ...
    */
    ...
    function updateGameSituation($card, $transferInfo) {
        ...
        if ($location_to == 'achievements') { // The player has got an extra-achievement
            // The number of achievements is the BGA score (not to be confused with the definition of score in InnovationNormaljosh game)
    ...
    function notifyWithTwoPlayersInvolved($card, $transferInfo, $progressInfo) {
        ...
        if ($owner_from == 0 || $owner_to == 0) { // Used only for Rocketry or Fission or Mass media
            ...
            switch($location_from . '->' . $location_to) {w
            case 'hand->hand':
    ...
    function getPlayerScore($player_id) { // Player InnovationNormaljosh score is different from the BGA score (number of achievements)
        return self::getUniqueValueFromDB(self::format("
        SELECT
            player_innovationnormaljosh_score
        FROM
            player
        WHERE
            player_id = {player_id}
    ...
    function getPlayerNumberOfAchievements($player_id) { // Player InnovationNormaljosh score is different from the BGA score (number of achievements)
        return self::getUniqueValueFromDB(self::format("
        SELECT
            player_score
        FROM
    ...
    function updatePlayerScore($player_id, $delta) {
        self::DBQuery(self::format("
            UPDATE
                player
            SET
                player_innovationnormaljosh_score = player_innovationnormaljosh_score + {delta}
            WHERE
                player_id = {player_id}
        ",
    ...
    function promoteScoreToBGAScore() {
        // Called if the game ends by drawing. The innovationnormaljosh score is the main value to check to determine the winner and the number of achievements is used as a tie-breaker.
        ...
        if (self::decodeGameType(self::getGameStateValue('game_type')) == 'team') {
            ...
            UPDATE
                ...
                LEFT JOIN (
                    SELECT
                        player_team, SUM(player_innovationnormaljosh_score) AS team_score
                    FROM
                        player
                    GROUP BY
                        player_team
            ...
            SET
                a.player_innovationnormaljosh_score = b.team_score
            ");
        }
        ...
        SET
            ...
            player_score = player_innovationnormaljosh_score
        ");
    }
    
    function binarizeBGAScore() {
        // Called if the game ends by dogma. The innovationnormaljosh score is 1 for winners, 0 for losers. There is no tie-breaker.
        self::DbQuery(self::format("
        UPDATE
            player
        SET
    ...
    function removeAllHandsBoardsAndScores() {
        ...
        self::DbQuery("
            UPDATE
                player
            SET
                player_innovationnormaljosh_score = 0,
...
//////////// 
    ...
    /*
        ...
        (note: each method below must match an input method in innovationnormaljosh.action.php)
    */
